import { useEditPostMutation } from "@/api/use-posts";
...
export default function EditPost() {
  ...
  const contentRef = useRef<string>(params.content as string);
  ...
  const handleEditPost = async () => {
    ...
    mutation.mutate({
      ...
      onSuccess:()=>{
        ...
        router.canGoBack()
          ? router.back()
          : router.push("/(app)/(authenticated)/(tabs)/userblogs");
      },
      onError: (error) => {
    ...
    <ScreenWrapper style={{ paddingTop: 0 }}>
        ...
        options={{
          ...
          headerRight: (props) => (
            <Button
              ...
              size="lg"
            >
              <CustomText className="text-white">{mutation.isPending ? "Saving..." : "Save"}</CustomText>
            </Button>
          ),
        }}
      />
      ...
      {mutation.isPending && (
        <View className="flex-1 justify-center items-center bg-black/50 absolute top-0 left-0 right-0 bottom-0 z-50 inset-0">
          <CustomText>Updating...</CustomText>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{paddingVertical:20}}>
        <View className="gap-4">
          ...
          <TagInputs tags={newPost.tags || ['trending']} setTags={(newTags) => setNewPost(prev => ({ ...prev, tags: typeof newTags === 'function' ? newTags(prev.tags) : newTags }))} maxTags={5}/>
          ...
          <PostImagePicker
            ...
            deafaultImage={params.image as string}
        ...
        <TextEditor contentRef={contentRef} initialValue={params.content as string} />
      </ScrollView>
    </ScreenWrapper>
  );
}
