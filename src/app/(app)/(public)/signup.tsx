...
GoogleSignin.configure({
  webClientId : "271833015585-k26t2mhf4s9anght0ljcm9rifb75phdq.apps.googleusercontent.com",
});
    ...
    await GoogleSignin.hasPlayServices();
     const response = await GoogleSignin.signIn();
       if (isSuccessResponse(response)) {
         ...
         const rsp = await googleLogin(response.data.user.email,response.data.user.photo!,response.data.user.name!)

         if (rsp.status === 200){
          const { accessToken, existingUser, refreshToken } =
            rsp.data as LoginMutationResponse;
          login({ currentUser: existingUser, accessToken: accessToken, refreshToken: refreshToken });
         }
       } else {
      ...
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        <View className="flex-1 justify-between pb-5">
          ...
          <View className="w-full px-5 justify-end mb-6">
            ...
            <Animated.View entering={FadeInUp.delay(900).duration(1000).springify()} className="mt-8 items-center">
              <CustomText className="dark:text-gray-300 text-gray-600 mb-6">
                Or continue with
              </CustomText>

                <TouchableOpacity className="bg-gray-200  dark:bg-neutral-800 items-center p-4 rounded-md mt-6 " onPress={handleGoogleLogin}>
                  <AntDesign name="google" size={24} color={colorScheme === "dark" ? "white" : "black"} />
                </TouchableOpacity>
            </Animated.View>
          </View>
